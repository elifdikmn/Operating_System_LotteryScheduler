...
#include <linux/modifiedscheduler.h> /*phase 2*/
...
repeat_schedule:
    ...
    {
        int total_ticket_number=0;
        int random_ticket;
        list_for_each(tmp, &runqueue_head) { // iterate throw the runnable processes
			p = list_entry(tmp, struct task_struct, run_list);
			if((((10*jiffies)-(10*p->last_time))>100)&&((p->ticket_number)!=10))
			{
				p->ticket_number = p->ticket_number+1;
			}
			else if((((10*jiffies)-(10*p->last_time))<10)&&((p->ticket_number)!=1))
			{
				p->ticket_number = p->ticket_number-1;
			}			
			total_ticket_number=total_ticket_number+(p->ticket_number);
		}
        get_random_bytes(&random_ticket, sizeof(random_ticket));
		if(random_ticket<0)
		{
			random_ticket=random_ticket*(-1);
		}
        if(total_ticket_number!=0){
			random_ticket=(random_ticket % total_ticket_number)+1; // get a random number between 1 and total_ticket_number
		}
        list_for_each(tmp, &runqueue_head){
			p = list_entry(tmp, struct task_struct, run_list);
			if (can_schedule(p, this_cpu)){
				random_ticket = random_ticket-(p->ticket_number);
				if(random_ticket<=0){
					next=p;
					break;
				}				
